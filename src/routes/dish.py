...
@router.get(
    ...
    responses={200: {"model": List[DishOutSchema]}},  # Примеры схем ответов для документации
)
...
async def create_dish(
    ...
    if submenu:
        created_dish = await DishService.create(submenu_id=submenu_id, new_dish=new_dish, session=session)
        return created_dish
    else:
